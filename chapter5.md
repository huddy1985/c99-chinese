...
源码字符集可能包含多字节字符集，用来显示一些扩展字符。

执行字符集也可以包含多字符集，并且不必要一定于源码字符集采用同样的编码方式。

但是这两种字符集要遵循以下的规则：

    应该都包含基本字符集，并且都是采用单字节编码
    显示、含义、以及展现的方式都依据locale而定
    多字符集可能具有依赖状态编码（state-dependent encoding），这种字符集以一个initial shift state开头，当遇到另外一个特定字符集的时候，该字符集由另外一个local-specific指定的shift states。
    当处于initial shift state时，所有的单字节字符不产生任何的改变。
    序列中解释后续子序列的字节是解释器根据当前转换状态的一个功能。
    一个所有位都为0的字节应该被解释为一个null字符，这个解释无视当前的转换状态。该字节不应该出现在任何的多字符集中。
    
对源文件而言，需要遵循以下的规则：
    
    一个标识符、注释、字符串、字符常量或者header name应该开始于initial shift state，并结束于该状态。
    并且他们该具有合法的多字符集。
    
###5.2.2 Character display semantics
active position表示输出设备写一个通过fputc函数将写入的位置。写入字符的目的是载显示上显示该字符的图形，并将active position移动到下一个位置。

写入的方向是locale-specific的。如果写入的active position位于一行的末尾，那么显示设备的行为是不定的。

转译后的字母序列表示无法显示的字符，用来在执行字符集中产生行为：

    \a alert				产生一个可以听到或者看到的警告，不改变active position
    \b backspace 	将active position移动到当前行的前一个位置。如果active position在一行的开始，那么该行为是未定的。
    \f form feed		将active position移动到下一个逻辑页的行首
    \n next line			将active position移动到下一行的行首
    \r carriage return	将active position 移动到当前行的行首。
    \t horizontal tab	移动active position到当前行下一个行制表符位置。如果当前active position位置下个位置无法存储一个行制表符，那么该行为未定的。
    \v vertical tab	与上个制表符行为类似。
    
上述的所有转移字符都应该产生一种实现定义的行为，并能够保存在一个字符对象中。

###5.2.3 信号和中断
函数应该被实现为能够在任何时间被信号打断或者被信号处理函数调用，或者两者都可以，该函数不会被提前修改，在中断之后仍然能够继续执行，并返回值，保存对象到硬盘。所有这些对象应该被外部的函数镜像维护。

###5.2.4 Environmental limits
翻译和执行环境限制了语言翻译和库的实现。下文总结了语言相关的环境限制。库的限制将在第七章讲解。

####5.2.4.1 Translation limits
本地实现应该能够翻译和执行一个程序并包含一个实例。

    127级的嵌套数据块
    条件语句嵌套层数63
    在同一个声明中可以包含12个指针、数组和函数声明，修改一个算法、结构体、联合和不完全类型。
    一个完整声明中可以最多嵌套63个括号
    一个完整语句中可以包含至多63级的嵌套括号
    内部标示符或者宏最长是63个字符				同一个文件的标示符
    外部标示符最长有效字符个数是31个			不同文件的标示符，需要链接
    在一个转译单元中最多有4095个外部标示符
    在一个块范围内至多只能申明511个标示符
    在一个预处理转译内，至多只能声明4095个宏标示符
    函数中至多只能有127个参数
    函数调用至多传递127个实参
    一个宏至多有127个参数
    宏调用至多传递127个实参
    一个逻辑源码行至多多有4095个字符或者宽字符
    一个对象至多有65535个字节（视环境实现而定）
    #include引用头文件至多嵌套15级
    switch之多只能有1023个case 标签
    单个struct或者union之多有1023个成员
    当一枚举只能有1023个枚举对象
    在声明结构体的时候，之多只能有63级的递归申明。
    
####5.2.4.2 Numerical Limits
所有的限制被文档在`<limits.h`>和`<float.h`>还有`<stdint.h`>

#####5.2.4.2.1 Size of interger types `<limits.h`>
